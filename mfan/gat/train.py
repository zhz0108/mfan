...
adj = normalize_symmetric(adj + torch.eye(adj.shape[0]), check_zero=True)
features = normalize_row(features, check_zero=True)

idx_train = torch.tensor(idx_train)
if args.dataset != 'facebook' and args.dataset != 'wiki':
    idx_val = torch.tensor(idx_val)
else:
    idx_val = torch.tensor(idx_train)
