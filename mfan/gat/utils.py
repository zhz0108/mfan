...
from torch_geometric.datasets import AttributedGraphDataset, WikipediaNetwork
...
def log(s: str, file: TextIO, print_console: bool = False):
    file.write(s)
...
def normalize_symmetric(A: torch.Tensor) -> torch.Tensor:
...
# noinspection PyPep8Naming
def normalize_row(A: torch.Tensor) -> torch.Tensor:
    D = torch.sum(A, dim=1, keepdim=True)
...
# noinspection PyPep8Naming
def normalize_symmetric2(A: torch.Tensor, check_zero: bool = False) -> torch.Tensor:
    D = torch.sum(A, dim=1, keepdim=True)
...
# noinspection PyPep8Naming
def normalize_row2(A: torch.Tensor, check_zero: bool = False) -> torch.Tensor:
    D = torch.sum(A, dim=1, keepdim=True)
...
def load_data21(dataset_str):
    if dataset_str == 'wiki':
        ...
        data = WikipediaNetwork(root='/tmp', name='chameleon').data
